...
export let streakCount = 1;
export let todayStudiedCount = 0;
export let DAILY_GOAL_TARGET = parseInt(localStorage.getItem('systemalogos_daily_goal_target'), 10) || 20;
...
export function setDailyGoalTarget(goal) {
  DAILY_GOAL_TARGET = goal;
  localStorage.setItem('systemalogos_daily_goal_target', goal);
}

export function loadStreakData(onUpdateUI) {
  try {
...
export function registerCardStudied(onUpdateUI, onTriggerConfetti) {
  todayStudiedCount++;
  updateAchievementProgress('flashcard_scholar', 1);
  updateAchievementProgress('daily_dedicated', 1);
...
export function updateAchievementProgress(id, increment = 1) {
  if (achievements[id]) {
...
export function renderAchievementsGrid() {
  ...
  const states = srsEngine.getAllCardStates();
  let masteredCount = 0;
