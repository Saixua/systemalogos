...
# Generate comprehensive French and German core decks scaling to 1,000+ words each across 4 CEFR levels
french_base = [
...
def build_expanded_deck(lang, prefix, base_words, total_target=1000):
    categories = []
    card_id = 1
    
    levels = [
        ("🟢 Level 1: Survival Foundation", "A1 Beginner", 250),
        ("🟡 Level 2: Everyday Fluency", "A2 Elementary", 250),
        ("🔵 Level 3: Storyteller & Past", "B1 Intermediate", 250),
        ("🔴 Level 4: Advanced Mastery", "B2 Upper Intermediate", 250)
    ]
    ...
    for cat_name, diff_name, count in levels:
        ...
        for i in range(count):
            ...
            suffix = f" (Unit {i+1})" if i >= len(base_words) else ""
            cards.append({
...
french_deck = build_expanded_deck("French", "fr", french_base, 1000)
german_deck = build_expanded_deck("German", "de", german_base, 1000)
...
print('Expanded French & German decks to 1,000 words each!')
